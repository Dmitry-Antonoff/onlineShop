...
module.exports = function AdminPage(props) {
  const { allUsers, page, limit, search } = props;
  return (
    <Layout {...props}>
      <main className="admin-main">
        ...
        <div className="administrator">
          ...
          <div className="back-next">
            <a
              href={`/admin?${search ? `search=${search}&` : ''}page=${+page > 1 ? +page - 1 : +page}`}
            >
              <img src="/svg/left.svg" alt="back" />
            </a>
            {+page >= 3 && (
              <a href={`/admin?${search ? `search=${search}&` : ''}page=${+page - 2}`}>{+page - 2}</a>
            )}
            {+page >= 2 && (
              <a href={`/admin?${search ? `search=${search}&` : ''}page=${+page - 1}`}>{+page - 1}</a>
            )}
            <span>...</span>
            {+page === limit || (
              <a href={`/admin?${search ? `search=${search}&` : ''}page=${+page + 1}`}>{+page + 1}</a>
